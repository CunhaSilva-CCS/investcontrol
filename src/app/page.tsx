...
export default async function DashboardPage() {
  ...
  const totalPrincipal = projections.reduce((sum, p) => sum + p.proj.principal, 0);
  const totalNetValue = projections.reduce((sum, p) => sum + p.proj.netValue, 0);
  ...
  for (const { inv, proj } of projections) {
    byType.set(inv.type, (byType.get(inv.type) ?? 0) + proj.netValue);
  }
